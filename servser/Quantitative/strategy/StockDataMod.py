...
def GetStockDatApi(stockName=None,stockTimeS=None,stockTimeE=None):
    path = '/Users/yupeng12/Desktop/my/GraduationProject/servser/Quantitative/strategy/StockDataCSV/'
...
def GetStockDatPro(stockName=None,stockTimeS=None,stockTimeE=None):
    ...
    for day_ind in range(xd, len(date)):
        if (stockPro.High[day_ind-xd:day_ind+1].max()-stockPro.Low[day_ind-xd:day_ind+1].min()) == 0:
            RSV[date[day_ind]] = RSV[date[day_ind - 1]]
        else:
