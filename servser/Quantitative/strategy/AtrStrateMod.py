...
class QuantPickTimeSys:
    ...
    def run_factor_plot(self):
        ...
        benchmark_trend = self.kl_pd[['benchmark_profit','trend_profit']].cumsum()
        plt.xticks([])  #去掉纵坐标值
        ...
        factor_result = {
            'Close': json.loads(self.kl_pd.Close.to_json(orient='index')),
            'Ma30': json.loads(self.kl_pd.Ma30.to_json(orient='index')),
            'artwin': json.loads(self.kl_pd.artwin.to_json(orient='index')),
            'artloss': json.loads(self.kl_pd.artloss.to_json(orient='index')),
            'dispCont_List': dispCont_List,
            'benchmark_trend': json.loads(benchmark_trend.to_json(orient='index')),
